            ...
            <section class="panel">
                <div class="panel-body">    
                    <form method="POST" action="{{ route('clientes/update', $client->id) }}">                  
                        ...
                        <div>
                        <input class="form-control" required="required" name="name" type="text" id="name" value="{{ $client->name }}"> 
                        <br>
                        <div class="form-group">
                            <label for="city_id">Selecciona una ciudad:</label>
                            <select class="form-control" id="city_id" name="city_id">
                                @foreach ($cities as $city)
                                    <option value="{{ $city->id }}" {{ ($city->id===$client->city_id)? 'selected=selected' : '' }}>{{ $city->name }}</option>      
                                @endforeach
                            </select>
                        </div>
